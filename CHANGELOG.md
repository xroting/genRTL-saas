...
### 🧾 Billing页面UI重构与修复 - 支持用量统计、多语言和数据显示

**更新日期**: 2026-02-08

**修复的问题**:
1. ❌ Included Usage部分显示为空（即使有usage数据）
2. ❌ 页面中英文夹杂，未使用多语言系统
3. ❌ 当用户没有Stripe订阅时，页面无法显示任何数据

**根本原因分析**:
通过Debug模式的运行时日志分析，发现了以下问题：
- **问题1核心原因**: 当用户没有 `stripe_customer_id` 时，后端API直接返回空响应，不包含 `current_period_start/end` 字段
- **问题2**: 页面所有文本都是硬编码的英文或中文，未使用项目的i18n系统
- **问题3**: 前端收到的数据中 `currentPeriodStart: undefined`，导致无法查询usage数据

**调试过程**:
1. 添加前端和后端日志，确认API被正确调用
2. 发现前端收到的响应中 `currentPeriodStart` 和 `currentPeriodEnd` 为 `undefined`
3. 检查后端代码，发现在 `!stripe_customer_id` 分支中提前返回，没有查询usage数据
4. 浏览器Console显示 `hasIncludedUsage: false`，服务器日志显示 `GET /api/dashboard/billing 200`
5. 定位到第54-64行的提前返回逻辑是问题根源

**解决方案**:

1. **修复无Stripe订阅用户的数据查询** - `app/api/dashboard/billing/route.ts`
   - ✅ 在没有 `stripe_customer_id` 的情况下，自动使用最近30天作为统计周期
   - ✅ 查询 `usage_ledger` 表获取 included 和 on_demand 数据
   - ✅ 返回完整的数据结构，包含 `current_period_start` 和 `current_period_end`
   - ✅ 保持与有Stripe订阅用户相同的数据结构

2. **添加完整的多语言支持** - `lib/i18n/translations.ts`
   - ✅ 为billing页面添加28个新的翻译键
   - ✅ 支持英文和中文两种语言
   - ✅ 包含所有UI元素：标题、表头、按钮、状态文本等
   
   新增翻译键：
   ```typescript
   billingAndInvoices, manageSubscription, includedUsage, onDemandUsage,
   invoices, item, tokens, cost, includedInProPlus, total, included,
   noUsageInPeriod, cycleStarting, type, qty, subtotal, noOnDemandUsage,
   date, description, status, amount, invoice, view, noInvoicesYet,
   invoicesWillAppearAfterFirstBillingCycle
   ```

3. **重构前端页面** - `app/(dashboard)/dashboard/billing/page.tsx`
   - ✅ 导入并正确使用 `useTranslation` hook（解构 `{ t }`）
   - ✅ 将所有硬编码文本替换为 `t('key')` 调用
   - ✅ Included Usage section始终显示（即使无数据也显示空状态）
   - ✅ 修复 `t is not a function` 运行时错误
   - ✅ 保持原有的Cursor风格UI设计

**修改文件**:
- `app/api/dashboard/billing/route.ts` - 修复无订阅用户的数据查询逻辑（第54-148行）
- `lib/i18n/translations.ts` - 添加billing相关翻译键（第397-425行，第908-936行）
- `app/(dashboard)/dashboard/billing/page.tsx` - 实现多语言和UI优化

**数据流程**:
1. 后端检查是否有Stripe订阅：
   - 有订阅：使用订阅周期查询usage
   - 无订阅：使用最近30天查询usage
2. 查询 `usage_ledger` 表，按 `bucket` 字段分组（'included' 和 'on_demand'）
3. 按模型统计token使用量和成本
4. 返回完整的数据结构给前端
5. 前端根据用户语言设置自动切换界面语言

**已修复**:
- ✅ Included Usage现在会显示所有符合条件的usage数据
- ✅ 无Stripe订阅的用户也能看到最近30天的用量统计
- ✅ 页面完全支持中英文切换，无硬编码文本
- ✅ 修复了 `t is not a function` 错误
- ✅ 所有用户都能看到完整的billing信息

**Debug模式使用**:
- 使用运行时日志确认API调用和数据流
- 浏览器Console日志帮助定位前端数据接收问题
- 服务器终端日志确认后端逻辑执行路径
